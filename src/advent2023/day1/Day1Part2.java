...
public class Day1Part2 {
    ...
    public static final String[] WORDS = { "zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine" };
    ...
    public static void main( String[] args ) throws Exception {
        // iterate on each lines of this text block
        String[] lines = puzzleInput.split( "\n" ); // split the text block by newline character
        ArrayList<Integer> list = new ArrayList<>(); // create an array list to store the concatenated digits
        int sum = 0; // initialize a variable to store the sum of the list
        for ( String line : lines ) { // loop through each line
            // extract the first digit and the last digit of each line
            System.out.println( line );
            String firstDigit = findFirstCalibrationDigit( line ); // get the first character of the line
            System.out.println( "firstdigit:" + firstDigit );
            String lastDigit = findLastCalibrationDigit( line ); // get the last character of the line
            System.out.println( "lastdigit:" + lastDigit );
            // concatenate it
            String concat = "" + stringToInt( firstDigit ) + stringToInt( lastDigit ); 
            int num = Integer.parseInt( concat ); // convert the string to an integer
            // add it in a array list
            list.add( num ); // add the integer to the array list
            // sum the content of this list
            sum += num; // add the integer to the sum
        }
        System.out.println( "The array list is: " + list ); // print the array list
        System.out.println( "The sum is: " + sum ); // print the sum

    }


    // define a method that takes a string as a parameter and returns an int
    public static String findLastCalibrationDigit( String s ) {
        Map<String, Integer> lettersDigitPositionMap = new TreeMap<>();
        for ( String word : WORDS ) {
            lettersDigitPositionMap.put( word, s.lastIndexOf( word ) );
        }
        int maxValueLettersDigit = Collections.max( lettersDigitPositionMap.values() );
        String resultForLetterDigits = getLettersDigitFromMap( lettersDigitPositionMap, maxValueLettersDigit );
        int maxValuePureDigit = lastDigitIndex( s );
        String resultPureDigit = findLastDigit( s );
        return maxValueLettersDigit > maxValuePureDigit?resultForLetterDigits:resultPureDigit;
    }

    private static String getLettersDigitFromMap( final Map<String, Integer> lettersDigitPositionMap, final int lettersDigitPosition ) {
        String resultForLetterDigits = "";
        for ( Map.Entry<String, Integer> entry : lettersDigitPositionMap.entrySet() ) {
            if ( entry.getValue()
                      .equals( lettersDigitPosition ) ) {
                String key = entry.getKey(); // this will give the key for the matching value
                resultForLetterDigits = key;
                break; // this will exit the loop once a match is found
            }
        }
        return resultForLetterDigits;
    }

    public static int lastDigitIndex( String s ) {
        // create a pattern that matches the last digit in the string
        Pattern p = Pattern.compile( "\\d(?=[^\\d]*$)" );
        // create a matcher object that can find the pattern in the string
        Matcher m = p.matcher( s );
        // if the pattern is found, return the index of the match
        if ( m.find() ) {
            return m.start();
        }
        // otherwise, return -1 to indicate no digit was found
        else {
            return -1;
        }
    }

    public static String findFirstCalibrationDigit( String s ) {
        Map<String, Integer> lettersDigitPositionMap = new TreeMap<>();
        for ( String word : WORDS ) {
            lettersDigitPositionMap.put( word, s.indexOf( word ) == -1 ? 999999999 : s.indexOf( word ) );
        }
        int minValueLettersDigit = Collections.min( lettersDigitPositionMap.values() );
        String resultForLetterDigits = getLettersDigitFromMap( lettersDigitPositionMap, minValueLettersDigit );
        int maxValuePureDigit = firstDigitIndex( s );
        String resultPureDigit = findFirstDigit( s );
        return minValueLettersDigit < maxValuePureDigit?resultForLetterDigits: resultPureDigit;
    }

    public static int firstDigitIndex( String s ) {
    ...
    // define a method that takes a string as a parameter and returns an int
    public static String findFirstDigit( String s ) {
        ...
        for ( int i = 0; i < s.length(); i++ ) {
            ...
            if ( Character.isDigit( c ) ) {
                // convert the character to an int and return it
                return "" + c;
            }
        }
        // if no digit is found, return -1
    ...
    // define a method that takes a string as a parameter and returns an int
    public static String findLastDigit( String s ) {
        // loop through each character of the string from the end
    ...
    public static int stringToInt( String s ) throws Exception {
        ...
        try {
            ...
            // if the string is not a valid int, proceed to the next step
        }
        // create a map of words for numbers from zero to nine and their corresponding values
        ...
        words.put( "eight", 8 );
        words.put( "nine", 9 );
        // check if the string is a valid word for a number from zero to nine
        if ( words.containsKey( s.toLowerCase() ) ) {
            // return the corresponding value
            return words.get( s.toLowerCase() );
        }
        // if the string is not a valid word for a number, return -1
