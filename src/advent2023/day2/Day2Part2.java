...
public class Day2Part2 {
    ...
    public static void main( String[] args ) {
        ...
        for ( Game game : games ) {
            ...
            int maxBlue = 0;
            int power = 0;
            for ( List<CubeSet> list : game.values() ) {
                System.out.println( list );
                for ( CubeSet cbs : list ) {
                    if ( cbs.get( "red" ) != null && cbs.get( "red" ) > maxRed ) {
                        maxRed = cbs.get( "red" );
                    }
                    if ( cbs.get( "green" ) != null && cbs.get( "green" ) > maxGreen ) {
                        maxGreen = cbs.get( "green" );
                    }
                    if ( cbs.get( "blue" ) != null && cbs.get( "blue" ) > maxBlue ) {
                        maxBlue = cbs.get( "blue" );
                    }
                }
            }
    ...
    private static void checkBag( final List<CubeSet> list ) {
    }
    ...
    private static List<Game> readGames() {
        ...
        for ( String line : lines ) {
            ...
            Integer gameId = Integer.valueOf( lineParts1[0].split( " " )[1] );
            String[] lineCubeSet = lineParts1[1].split( ";" );
            Game game = new Game();
            List<CubeSet> cubeSets = new ArrayList<>();
            for ( String lineCubeSetItem : lineCubeSet ) {
                String[] lineCubePairSet = lineCubeSetItem.split( "," );
                CubeSet cubeSet = new CubeSet();
                for ( String lineCubePairSetItem : lineCubePairSet ) {
                    String[] lineCubeElements = lineCubePairSetItem.split( " " );
                    cubeSet.put( lineCubeElements[2], Integer.valueOf( lineCubeElements[1] ) );
                }
                cubeSets.add( cubeSet );
            }
            game.put( gameId, cubeSets );
